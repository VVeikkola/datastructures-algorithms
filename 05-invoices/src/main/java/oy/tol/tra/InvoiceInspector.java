...
public class InvoiceInspector {
   ...
   public void handleInvoicesAndPaymentsFast() {
      ...
      for(int counter = 0; counter < invoices.length && payments.length > 0; counter ++){
         Payment payment = new Payment(invoices[counter].number, 0);
         ...
         if (paymentIndex >= 0 && paymentIndex < payments.length){
            Payment foundPayment = payments[paymentIndex];
            if (invoices[counter].sum.compareTo(foundPayment.sum) > 0) {
               toCollect.add(new Invoice(invoices[counter].number, invoices[counter].sum - foundPayment.sum, dueDateValue));
            }
         }else {
            Invoice invoice = new Invoice(invoices[counter]);
            invoice.dueDate = dueDate.getTime();
            toCollect.add(invoice);
         } 
      }
   }
